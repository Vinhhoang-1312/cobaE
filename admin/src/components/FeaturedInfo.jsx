...
const FeaturedInfo = () => {
  ...
  const delivered = orders
    .map((element) => element.status === "Shipped")
    .reduce((a, b) => a + b, 0);
  ...
  const lastYearIncome = lastYearSales * 0.25; // fake data, different percentages
  const lastYearCost = lastYearSales * 0.9; //fake data, different percentages

  const currentYearSales = currentYearOrders
    .map((element) => element.amount)
    .reduce((a, b) => a + b, 0);

  const currentYearIncome = currentYearSales * 0.22; // fake data, different percentages
  const currentYearCost = currentYearSales * 0.11; // fake data, different percentages
